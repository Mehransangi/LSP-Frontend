    ...
    try {
      ...
      if (!abortController.signal.aborted) {
        setFileItems((prev) =>
          prev.map((item) =>
            item.id === fileId
              ? { ...item, status: "success", url: url, progress: 100 }
              : item))
...
export const ImageUploadNode = (props) => {
  ...
  // Override upload function to use base64
  React.useEffect(() => {
    extension.options.upload = handleImageUpload;
  }, [extension]);

  // Debug: log base64 image and editor content after upload
  React.useEffect(() => {
    if (fileItems.length > 0 && fileItems.some(item => item.status === "success" && item.url?.startsWith("data:image"))) {
      console.log("Base64 Image(s):", fileItems.map(item => item.url));
      setTimeout(() => {
        console.log("Editor HTML:", props.editor.getHTML());
      }, 100); // slight delay to ensure DOM update
    }
  }, [fileItems, props.editor]);
